...
//    public List<Article> findStyle();
    ...
    @Insert("insert into article(id,title,description,img,type,text,upload_time,update_time) values(#{id},#{title},#{description},#{img},#{type},#{text},#{display},#{uploadTime},#{updateTime})")
    public int insertArticle(Article article);

    @Update("update article set title=#{title},description=#{description},img=#{img},type=#{type},text=#{text},display=#{display},upload_time=#{uploadTime},update_time=#{updateTime} where id = #{id}")
